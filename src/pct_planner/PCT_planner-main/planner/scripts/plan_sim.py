...
parser.add_argument('--scene', type=str, default='Spiral', help='Name of the scene. Available: [\'Spiral\', \'Building\', \'Plaza\',\'Second\']')
...
if args.scene == 'Spiral':
    ...
    start_pos = np.array([-16.0, -6.0, 0.0], dtype=np.float32)
    end_pos = np.array([-26.0, -5.0, 0.0], dtype=np.float32)
elif args.scene == 'Building':
    tomo_file = 'MultifloorBuilding'
    start_pos = np.array([0.0, 7.5, 0.0], dtype=np.float32)
    ...
    start_pos = np.array([0.0, 0.0, 3.0], dtype=np.float32)
