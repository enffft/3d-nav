...
tomography_dir = os.path.join(script_dir, '..')
sys.path.insert(0, tomography_dir)
import time
import pickle
import numpy as np
...
class Tomography(object):
    ...
    def loadPCD(self, pcd_file):
        ...
        rospy.loginfo("Point cloud X range: [%.2f, %.2f]", self.points_min[0], self.points_max[0])
        rospy.loginfo("Point cloud Y range: [%.2f, %.2f]", self.points_min[1], self.points_max[1])
        rospy.loginfo("Point cloud Z range: [%.2f, %.2f]", self.points_min[2], self.points_max[2])
        rospy.loginfo("the value of slice_h0 is: [%.2f]", self.slice_h0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scene', type=str, help='Name of the scene. Available : [\'Spiral\', \'Building\', \'Plaza\', \'Second\']')
    arg = parser.parse_args()

    cfg = Config()
    scene_cfg = getattr(__import__('config'), 'Scene' + arg.scene)
